...
- **Под каким именем агент виден в трейсе.** Имя собирается лестницей, и каждая
  ...
  |---|---|---|
  | эксперимент | `emotional-glue` | `MLFLOW_EXPERIMENT` в env; по конвенции — имя выданного репозитория |
  | корневой спан | `run 20260804-142118` | `run_span(name=...)` |
  | спан шага | `10_passport` | `step_span(name=...)` |
  | **спан агента** | `invoke_agent 10_passport` | **`Agent(name=...)` в pydantic-ai** |
  ...
  паспортиста от верификатора (проверено на pydantic-ai 2.23). Если агент создаётся
  на каждый вызов, передавай туда имя шага — тогда имя шага и имя агента совпадут,
  и дерево читается без расшифровки.
- **Системный промпт, вход и ответ кладите в `inputs`/`outputs` спана шага —
  и НЕ через `Agent(system_prompt=…)`.** Без этого в трейсе видно только сообщение
  пользователя и ответ: промпт уезжает в атрибут `gen_ai.system_instructions`, куда
  никто не смотрит, — а именно он объясняет, почему шаг ответил так, а не иначе.
  ...
  сообщений агента. Бонус — MLflow подставляет их в колонки Request/Response списка
  трейсов, и прямо в списке видно, с каким промптом шаг работал и что ответил.
  Проверено на 16-шаговом конвейере, mlflow 3.15.1 + pydantic-ai 2.23.
...
4a. **Расставь спаны так, чтобы запуск был одним трейсом**: `run_span()` в точке
...
7. Проверь работой, а не чтением кода (AGENTS.md §7a). Многошаговый агент прогоняй
