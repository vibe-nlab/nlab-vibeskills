...
def step_span(name: str, session_id: str = "", inputs: dict | None = None, *,
    ...
    **Три опциональных поля контракта** (все — keyword, старые вызовы не ломаются):
    ...
    - `after` — id шагов, чьи РЕЗУЛЬТАТЫ пошли на вход этому. Единственный источник
      стрелок: MLflow хранит вложенность (`parent_span_id`), но не поток данных
      между соседями. Не проставили — вьюер обязан нарисовать шаги БЕЗ стрелок, а
      не соединять всё, что шло рядом: догадка по времени даёт другой граф, а не
      приближение (см. правило 4 контракта в SKILL.md).
