...
class Poller(threading.Thread):
    ...
    def __init__(self, base: str, experiment_id: str, known_before: set[str]):
        ...
        self.span_seen_at: dict[str, float] = {}
    ...
    def tick(self) -> None:
        ...
        for sp in spans:
            name = sp.get("name", "?")
            if name not in self.span_seen_at:
                self.span_seen_at[name] = el()
...
def known_traces(base: str, experiment_id: str) -> set[str]:
    ...
    resp = requests.post(
        f"{base.rstrip('/')}/api/3.0/mlflow/traces/search",
        json={
    ...
    return {t["trace_id"] for t in resp.json().get("traces", [])}


# --- что именно прогоняем ----------------------------------------------------


async def run_fake(local_done: dict[str, float]) -> None:
    ...
    with run_span("проба реалтайма", inputs={"режим": "заглушка"}) as out:
        for step, delay in (("1-шаг", 4), ("2-шаг", 5), ("3-шаг", 4)):
            with step_span(step, inputs={"вход": step}):
                await asyncio.sleep(delay)
            local_done[step] = el()
            print(f"[{el():>5}] шаг закрыт локально: {step}")
        out.update({"статус": "ok"})


async def run_real(local_done: dict[str, float]) -> None:
    """Прогон СВОЕГО конвейера — под проект правится здесь.

    Достаточно обернуть свою точку входа и после каждого шага записать
    `local_done[имя_шага] = el()`. Проще всего — временно подменить свою
    функцию шага обёрткой, как показано ниже.
    """
    raise SystemExit(
        "run_real не заполнен: подставьте сюда вызов своего конвейера "
        "(и отметку local_done[шаг] = el() после каждого шага)"
    )
...
async def main() -> None:
    ...
    if not setup_tracing(os.getenv("MLFLOW_EXPERIMENT", "realtime-probe")):
    ...
    experiment = mlflow.get_experiment_by_name(
        os.getenv("MLFLOW_EXPERIMENT", "realtime-probe")
    )
    ...
    for step, done_at in local_done.items():
        seen = poller.span_seen_at.get(step)
