    ...
    const mutationHandler = (mutations) => {
      ...
      newStyleEntries.forEach(({ element, href }) => {
        ...
        if (styleExists) {
          ...
          element.setAttribute('data-fouc-route', window.location.pathname);
          element.removeAttribute("data-n-href");
          fixedStyleHrefs.push(href);
        }
      });

      const styles = document.querySelectorAll('[data-fouc-route]');
      if (styles.length > 1) {
        const currentStyle = document.querySelector(`[data-fouc-route="${window.location.pathname}"]`);
        const lastStyle = styles[styles.length - 1];
        if (currentStyle && lastStyle) {
          const parentEl = currentStyle.parentNode;
          parentEl.insertBefore(currentStyle, lastStyle.nextElementSibling);
        }
      }
